...
- `reset` — очистить БД и заново загрузить demo-данные (локальная разработка/повторяемые прогоны).
- `once` — загрузить demo-данные только если БД пустая (без перезаписи существующих данных).
- `off` — не загружать demo-данные.
...
2. Войти под `admin`, открыть `/admin/dashboard-v2` и показать KPI/нагрузку.
3. Войти под `methodist`, открыть `/methodist/queue` и показать очереди документов/доступа.
4. Войти под `teacher`, открыть `/teacher/groups`, обновить прогресс и завершить обучение.
5. Войти под `student`, открыть `/student/cabinet` и показать этап, прогресс, рекомендации, документы.
6. Пройти цепочку: программа -> заявка -> оплата -> аттестация -> удостоверение.
7. Показать ролевые ограничения на одном из экранов.
8. Вызвать контролируемую ошибку и показать единую страницу обработки.
9. Завершить ссылкой на GitHub и разделом с диаграммами/ПЗ.
